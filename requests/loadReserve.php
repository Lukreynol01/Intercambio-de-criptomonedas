...
$query2 = $db->query("SELECT * FROM ec_currencies WHERE cid='$row[id]' and company_from='$from' and company_to='$to' and currency_from='$currency_from' and currency_to='$currency_to' ORDER BY id");
if($query2->num_rows>0) { 
	$row2 = $query2->fetch_assoc();
	echo $row2['reserve']." ".$row2['currency_to'];
} else {
	echo '-';
}
?>